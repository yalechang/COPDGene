...
in the selected features. Instead of KMeans, we use Gaussian Mixture Model to
do clustering.
"""
print __doc__
...
for i in range(len(bfs)):
    ...
    for n_components in n_components_range:
        ...
        gmm_1 = mixture.GMM(n_components=n_components,covariance_type='full')
        gmm_1.fit(data_use)
        data_use_labels = gmm_1.predict(data_use)
        bic_1.append(gmm_1.bic(data_use))

        gmm_2 = mixture.GMM(n_components=n_components,covariance_type='full')
        gmm_2.fit(data_ref_use)
        data_ref_use_labels = gmm_2.predict(data_ref_use)
        bic_2.append(gmm_2.bic(data_ref_use))
    for i in range(len(bic_1)):
        if bic_1[i] == min(bic_1):
            n_clusters_1 = i+1
        if bic_2[i] == min(bic_2):
            n_clusters_2 = i+1
